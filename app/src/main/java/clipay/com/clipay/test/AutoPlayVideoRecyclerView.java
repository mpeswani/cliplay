package clipay.com.clipay.test;
...
public class AutoPlayVideoRecyclerView extends RecyclerView {
    ...
    private BaseViewHolder handingVideoHolder;
    ...
    private void initView(Context context) {
        ...
        Disposable disposable = subject.debounce(300, TimeUnit.MILLISECONDS)
                .filter(value -> true)
        ...
        addOnScrollListener(new OnScrollListener() {
            @Override
            public void onScrolled(@androidx.annotation.NonNull RecyclerView recyclerView, int
                    dx, int dy) {
    ...
    private void playVideo(float value) {
        Observable.just(value)
                .map(aFloat -> {
                    BaseViewHolder videoHolder = getViewHolderCenterScreen();
                    if (videoHolder == null) return null;
                    if (videoHolder.getItemViewType() == MultipleItem.TEXT || videoHolder
                            .getItemViewType() == MultipleItem.IMG) {
                        return null;
                    }
                ...
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<BaseViewHolder>() {
                    @Override
                    public void onSubscribe(@NonNull Disposable d) {
                    }

                    @Override
                    public void onNext(@NonNull BaseViewHolder videoHolder) {
                        if (handingVideoHolder != null) {
                            handingVideoHolder.stopVideo();
                            Log.e("play", "===>+ video stop");
                        }
                        videoHolder.playVideo();
                        Log.e("play", "===>+ video play");
    ...
    private BaseViewHolder getViewHolderCenterScreen() {
        ...
        for (int i = min; i <= max; i++) {
            ...
            if (percentViewHolder > percentMax && percentViewHolder >= 50) {
                ...
                viewHolderMax = (BaseViewHolder) viewHolder;
                newPosition = i;
            }
        }
    ...
    private int[] getLimitPositionInScreen() {
        int findFirstVisibleItemPosition = ((LinearLayoutManager) getLayoutManager())
                .findFirstVisibleItemPosition();
        int findFirstCompletelyVisibleItemPosition = ((LinearLayoutManager) getLayoutManager())
                .findFirstCompletelyVisibleItemPosition();
        int findLastVisibleItemPosition = ((LinearLayoutManager) getLayoutManager())
                .findLastVisibleItemPosition();
        int findLastCompletelyVisibleItemPosition = ((LinearLayoutManager) getLayoutManager())
                .findLastCompletelyVisibleItemPosition();
        int min = Math.min(Math.min(findFirstVisibleItemPosition,
                findFirstCompletelyVisibleItemPosition),
                Math.min(findLastVisibleItemPosition, findLastCompletelyVisibleItemPosition));
        int max = Math.max(Math.max(findFirstVisibleItemPosition,
                findFirstCompletelyVisibleItemPosition),
