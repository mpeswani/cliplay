...
class QuickAdapter(list: List<MultipleItem>, context: Context, player: SimpleExoPlayer) : BaseMultiItemQuickAdapter<MultipleItem, BaseViewHolder>(list) {
    ...
    override fun convert(viewHolder: BaseViewHolder, item: MultipleItem) {
        when (item.itemType) {
            ...
            MultipleItem.VIDEO -> {
                ...
                view.setResizeMode(AspectRatioFrameLayout.RESIZE_MODE_FIXED_HEIGHT)
                view.player = player
